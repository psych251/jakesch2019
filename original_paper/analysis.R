...
ggplot(plot_d31, aes(x=condition, y=rating, colour=type, group=type)) + #geom_line(position=pd) + 
  ...
  geom_point(position=pd, size  = 4) + theme_bw(base_size = 15) + xlab('Condition') + ylab('Mean trustworthiness') +
  ...
  labs(color='Profile type') +  scale_color_manual(values=colors) + ylim(60,75)
...
ggplot(plot_d32, aes(x=aiscore_r, y=rating)) + ylim(50,82) +
...
original_plot_d31 = plot_d31

original_plot_d32 = plot_d32

save(original_processed_data, original_plot_d31, original_plot_d32, file = "original_data.RData")
