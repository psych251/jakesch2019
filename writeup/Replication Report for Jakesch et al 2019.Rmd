...
#### Actual Sample
We recruited 170 participants, as planned.

#### Differences from pre-data collection methods plan
There were no significant differences between the pre-planned methods and confirmatory analysis.
...
```{r}
responses <- read.csv(file="../../jakesch_et_al_2019_replication_survey_data.csv") %>% 
...
original_processed_data <- original_processed_data %>% rename(ai = type, meanScore = rating,
...
# revalue the columns in the data frame to make plotting more clear
...
plot_data <- function (data, title) {
  ...
  ggplot(data, aes(x=ai,y=meanScore)) + 
    ...
    theme_cowplot() + facet_grid(~condition) + ggtitle(title)
}
...
Let's do a small fixed effects meta-analysis with the results on the key statistical test of interest from the original study and our replication.

Because this is a direct replication, a fixed effects meta-analysis is most appropriate (because the two studies were effectively identical, we assume that there is one true effect that is measured in both studies, and that all observed variation is due to sampling error).
...
forest.meta(meta_analysis,spacing=1.0,lineheight=unit(3,"cm"))

```

We see that when we pool the evidence from both studies, there is still aggregate evidence that the Replicant Effect (operationalized as the AI x Condition interaction term in the mixed-effects model) holds in this scenario.
...
Now let's try to replicate the other figure for Experiment 3
```{r}
...
ai_scores = read.csv(file="../../jakesch_et_al_2019_replication_survey_data.csv") %>%
  filter(!between(row_number(),1,2)) %>% 
...
normalized_responses_with_ai_scores = inner_join(processed_data,ai_scores,by=c("ResponseId","profile_number")) %>%
  inner_join(participant_average_rating, by=c("ResponseId")) %>% 
  mutate(normalized_score = meanScore - meanRating) 
...
#ggplot(normalized_responses_with_ai_scores, aes(x=ai,y=ai_score,fill=ai)) + geom_flat_violin()
...
p2 <- plot_figure_1(plot_d31, "Replication")

#ggsave("repl_main_plot.png",height=4,width=4*1.2)
...
# ggplot(processed_data, aes(x=condition,y=meanScore)) + geom_jitter(color="grey",alpha=0.8,width=.2) +  stat_summary(fun.y="mean", geom="line", size=1, aes(group = 1)) + stat_summary(fun.y=mean, geom="point",size=2.5) + ylim(0,100)  + facet_wrap(~profile_number,ncol=15) + theme_cowplot() + theme(axis.text.x = element_text(angle = 45,  hjust = 1))

# ggsave("plot.png",height=9,width=9*1.61)


slope_plot <- function(data) {
  ggplot(data, aes(x = condition, y = meanScore)) + 
...
slope_plot(processed_data)
slope_plot(original_processed_data %>% filter(condition %in% c("control","primed")))
...
Based on the pre-registered key statistic of interest, we must consider this replication to have failed. 

While we still see a difference in the raw data between mean trustworthiness ratings for the human and AI profiles in the primed condtion (XXX vs SSS), the `AI*condition` in the linear mixed effects model is not signficant (Beta =, SD =, p = ). The relevant coefficient is in the same direction as, but smaller in magnitude than, the coefficient in the original paper (Beta =, SD = , p = ).

One posible explanation for this is that the observed trustworthiness values in the control condition are significantly lower than those observed in the original paper. The difference is significant enough that it seems unlikely to be purely due to sampling error. It is possible that this reflects a change in opinion.

[exploratory analysis -- did general trustworthiness / AI score vary between the two studies?]

[interpretation of interaction effects]

When we run a model without random effects (equivalent to model 1 in the original paper), we see that the AI term is significant (unlike in the origial study]. This indicates that, ignoring 


### Commentary
This replication 

The meta-analysis shows that it is likely the Replicant Effect does exist, though it is likely smaller in magnitude than the what was reported in the original study.

This is supported by the observed inverse relationship between AI score and trustworthiness rating. This provides strong evidence that. However, unlike the original study, we did not observe any floor effect, instead observing a linear relationship.

This may be an example of the "Winner's curse", which states that, purely how. This effects even a pre-registered and well-designed study such as the one replicated here. 

To confirm whether the failure to replicate was due to to sampling error and a lack of statistical power or whether there have been shifts in the qualities, a

We should also consider that the other two experimental conditions from the original experiment. These conditions were more ecologically valid, but had smaller effects on trustworthiness ratings than the primed condition. The new results suggest that a replication of these two conditions should also be considered.
