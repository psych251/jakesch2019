...
To be conducted on Monday Oct 29 after power analysis is covered in class.
...
We contacted the authors to ask for the exact wording of their attentiveness tests (since it is not clear from the source they refer to which exact linguistic attentiveness check was used).
...
Then, we fit a generalized linear mixed model (GLMM) with random per-participant effects to each participants' trustworthiness ratings: `lme(meanScore ~ ai * condition, random=~ 1|subject)`.
...
```{r}
responses <- read.csv(file="pilot_data_7_responses.csv") %>% 
...
control_timing_data <- responses %>% gather(time_interval, time, contains("timing_"), -contains("Click")) %>% select(time_interval,time,ResponseId) %>% group_by(ResponseId) %>% summarize(meanRatingTime = mean(as.numeric(time),na.rm=T)) %>% filter(meanRatingTime > 0)
...
primed_timing_data <- responses %>% 
  ...
  summarize(meanRatingTime = mean(as.numeric(time),na.rm=T)) %>% filter(meanRatingTime > 0)

# combine both sets of timing data
timing_data = rbind(control_timing_data,primed_timing_data)

participants_to_exclude_based_on_timing <- timing_data %>% filter (meanRatingTime < 5.0) %>% select(ResponseId)
...
I obtained seven pilot responses. Two were from others (I posted a link to my Qualtrics survey on Facebook) and five were from myself. One is excluded from the analysis because of the low standard deviation of the responses. 

Using the pilot data, we see no significant terms in the GLM model. When I ran an initial pilot with two responses generated by me (one per condition), where I consciously tried to put in data that would match the hypothesis, the model didn't give any signficant results, but did have coefficients with the magnitudes and signs that I expected. (Beta_intercept = 75.3, Beta_ai = 8.67, Beta_primed = 10.0, Beta_primed_x_ai = 57.3 -- see the data in pilot_data_2_responses.csv).

[*Single test and justification*: If we find that the AI x Primed interaction is significant and has a negative Beta coefficient, then we consider the experiment to have successfully been replicated - since this indicates that AI-like profiles were rated less trustworthy in the treatment condition (where participants are told some profiles are AI-generated) than in the control (where they believe all profiles to be human-generated). Otherwise, we consider the experiment to not have successfully been replicated.]
