...
See https://stanforduniversity.qualtrics.com/jfe/form/SV_a4WTJ0FvjqS5vJX
for the Qualtrics survey.

And https://stanforduniversity.ca1.qualtrics.com/Q/EditSection/Blocks?ContextSurveyID=SV_a4WTJ0FvjqS5vJX for the survey code (shared with Julie)

See https://workersandbox.mturk.com/?filters%5Bsearch_term%5D=jacob+ritchie for the Amazon Mechanical Turk listing (on the Requester Sandbox - for testing purposes only).
...
Using the 'simr' package, we determined that using the original sample size would provide 89.70% power (95% CI: [87.65, 91.51]) for the fixed effect of interest (see below), assuming participants are excluded at similar rates to the original study. Power analysis code is located in the github repository in the power_analysis.R file, and results of a power curve analysis are reported in power_curve.txt


### Planned Sample

Based on the power analysis, we planned to recruit 85 participants per condition (slightly above the number in the original sample), which should give above 90% power to retrieve the effect of interest.
...
Update: this has been fixed ahead of pilot B.
...
```{r}
responses <- read.csv(file="pilot_b_test_data_2_responses.csv") %>% 
  ...
  # currently commented out because in one of my two test runs 
  # I forgot to complete the attention check
  # 
  #     filter(
